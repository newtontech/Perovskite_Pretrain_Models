...
class MolPredict(object):
    ...
    def predict(self, data, save_path=None, metrics='none'):
        ...
        if self.save_heatmap:
            save_dir = data.replace('.csv','_heatmap')
            # make folder
            if not os.path.exists(save_dir):
                os.makedirs(save_dir)

        self.datahub = DataHub(data = data, is_train = False, save_path=self.load_model, save_heatmap = self.save_heatmap,save_heatmap_dir=save_dir,**self.config)
        # test
        ...
        if not (df[self.target_cols] == -1.0).all().all() and self.save_heatmap==False:
