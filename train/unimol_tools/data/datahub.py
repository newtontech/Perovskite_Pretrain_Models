...
class DataHub(object):
    ...
    """
    def __init__(self, data=None, is_train=True, save_path=None,save_heatmap=False,save_heatmap_dir=None, **params):
        """
        ...
        self.save_heatmap_dir = save_heatmap_dir
    ...
    def _init_data(self, **params):
        ...
        if params.get('model_name', None) == 'unimolv1':
            if 'atoms' in self.data and 'coordinates' in self.data:
                no_h_list = ConformerGen(**params).transform_raw(self.data['atoms'], self.data['coordinates'],self.save_heatmap_dir)
            else:
                smiles_list = self.data["smiles"]                  
                no_h_list = ConformerGen(**params).transform(smiles_list, self.save_heatmap_dir)
