...
class ConformerGen(object):
    ...
    def single_process(self, smiles, save_heatmap_dir=None):
        """
        ...
        if self.method == 'rdkit_random':
            ...
            return coords2unimol(atoms, coordinates, self.dictionary, self.max_atoms, remove_hs=self.remove_hs,save_heatmap_dir=save_heatmap_dir)
        else:
            raise ValueError('Unknown conformer generation method: {}'.format(self.method))
        
    def transform_raw(self, atoms_list, coordinates_list, save_heatmap_dir = None):

        inputs = []
        for atoms, coordinates in zip(atoms_list, coordinates_list):
            inputs.append(coords2unimol(atoms, coordinates, self.dictionary, self.max_atoms, remove_hs=self.remove_hs, save_heatmap_dir=self.save_heatmap_dir))
        return inputs

    def transform(self, smiles_list, save_heatmap_dir = None):
        logger.info('Start generating conformers...')
        if self.multi_process and save_heatmap_dir is None:
            pool = Pool(processes=min(8, os.cpu_count()))
            inputs = [item for item in tqdm(pool.imap(self.single_process, smiles_list))]
            pool.close()
        else:
            inputs = [self.single_process(smiles,save_heatmap_dir) for smiles in tqdm(smiles_list)]
...
def coords2unimol(atoms, coordinates, dictionary, max_atoms=256, remove_hs=True,save_heatmap_dir=None, **params):
    """
    ...
    if save_heatmap_dir is not None:
        import json
        # save atoms
        with open(os.path.join(save_heatmap_dir, 'atoms.json'), 'w') as f:
            json.dump(atoms, f)
    # print(atoms)
    # exit()
