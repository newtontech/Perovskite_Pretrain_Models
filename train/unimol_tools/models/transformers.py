...
class TransformerEncoderWithPair(nn.Module):
    ...
    def forward(
        ...
        # # print(x_norm.shape)
        # # exit()
        # print(i_attn_mask.shape)
        # print(attn_mask.shape)
        # # exit()
        # dir = "/data/yhm/test_mol/unimol_feature_origin_3.pt"
        # dir = "/data/yhm/test_mol/unimol_feature_4.pt"
        # mean_heatmap = torch.max(attn_mask, dim=-1).values
        if dir is not None:
            import os
            save_dir = os.path.join(dir, 'mean_heatmap.pt')
            mean_heatmap = attn_mask.mean(dim=-1)
            torch.save(mean_heatmap, save_dir)
        # exit()
