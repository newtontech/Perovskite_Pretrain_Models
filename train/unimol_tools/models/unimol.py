...
class UniMolModel(nn.Module):
    ...
    def __init__(self, output_dim=2, data_type='molecule', **params):
        ...
        if params.get('train_from_scratch'):
            self.load_pretrained_weights_r(path=self.pretrain_path)
        else:
            self.load_pretrained_weights(path=self.pretrain_path)
    def load_pretrained_weights_r(self, path):
        """
        Loads randomly initialized weights with the same shape as the pretrained model.

        :param path: (str) Path to the pretrained weight file (ignored for random initialization).
        """
        logger.info("Ignoring pretrained weights path and using random initialization")
        
        # Get the current model's state dict to see the shapes
        current_state_dict = self.state_dict()
        
        # Create a new state dict with random values but same shapes
        # random_state_dict = {}
        # scale = 0.1
        # for key, tensor in current_state_dict.items():
        #     random_state_dict[key] = torch.randn_like(tensor)*scale
        
        # # Load the random weights
        # save_path = "random_init_weights.pth"
        weight_dir = os.path.join(os.path.dirname(path), 'random_init_weights.pth')
        if not os.path.exists(weight_dir):
            print(f"Random weights not found at {weight_dir}, creating new ones...")
            # Create a new state dict with random values but same shapes
            random_state_dict = {}
            scale = 0.1
            for key, tensor in current_state_dict.items():
                random_state_dict[key] = torch.randn_like(tensor) * scale
            # Save the random weights to a file
            torch.save(random_state_dict, weight_dir)
        else:
            print(f"Random weights already exist at {weight_dir}, loading...")
            random_state_dict = torch.load(weight_dir)

        # torch.save(random_state_dict, save_path)
        # print(f"Random weights saved to {save_path}")
        # exit()
        print("Start training from scratch...")

        self.load_state_dict(random_state_dict, strict=False)
    ...
    def forward(
        ...
        **kwargs
        ...
        (
            ...
            _,
            _,
        ) = self.encoder(x, padding_mask=padding_mask, attn_mask=graph_attn_bias,dir=dir)
        ...
        # features_dir = '/data/yhm/unimol_demo/split_seed_40/point_feature.pt'
        # torch.save(cls_repr, features_dir)
        # print("feature saved")



        # exit()
        return cls_repr, logits
